...
Inductive SimpParamItrp D (G:Ctx D) F : list nat->forall T:forall d,G d->Type,(forall d g,F d->T d g)->Type :=
	spItrpNil : SimpParamItrp D G F nil (fun d g=>F d) (fun d g f=>f) |
...
Inductive SimpFamItrp D (G:Ctx D) : simpFam->(forall d,G d->Type)->Type :=
	sfItrpNil : SimpFamItrp D G nil (fun d g=>Type) |
...
Inductive SimpCtxItrp D : list (nat * list nat)->Ctx D->Type :=
	scItrpNil : SimpCtxItrp D nil (empCtx D) |
	scItrpCons f la G G' F (f':AtTCtx D f F) la' :
		SimpCtxItrp D G G'->
		SimpParamItrp G' F la (fun d g=>Type) la'->
		SimpCtxItrp D ((f,la) :: G) (extCtx G' (fun d g=>la' d g (tctxProj f' d))).
